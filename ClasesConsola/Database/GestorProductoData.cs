...
namespace AplicacionConsola.Database
{
    public class GestorProductoData
    {
        ...
        public static List<Producto> GetListaProducto()
        {
            ...
            throw new Exception($"Error, no se pudo obtener la lista de Productos");
        }
        ...
        public static bool UpdateProducto(int idProducto, Producto producto)
        {
            using (SqlConnection connection = new SqlConnection(stringConnection))
            {
                ...
                string query = "UPDATE  FROM Producto (Descripciones,Costo,PrecioVenta,Stock,IdUsuario) values (@descripcion,@costo,@precioVenta,@stock,@idUsuario) where id = @id;";
